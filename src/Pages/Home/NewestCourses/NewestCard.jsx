...
const NewestCard = ({ newest }) => {
    ...
    return (
        <div className="card card-compact md:p-2 max-sm:p-6 shadow-xl  bg-white rounded-xl  overflow-hidden w-full sm:w-[300px] md:w-[350px] lg:w-[420px] hover:scale-105 transform transition-all duration-300">
            <Link to={`/newest/${_id}`}>
                <figure className="relative">
                    <img
                        src={backgroundCardImage}
                        alt="Course Background"
                        className="w-full h-48 sm:h-56 md:h-64 lg:h-72 object-cover rounded-t-xl"
                    />
            ...
            <div className="card-body p-6">
                ...
                <div className='flex items-center gap-3 mb-4'>
                    <img className='w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 border-green-500' src={instructorImage} alt="Instructor" />
                    <h2 className="text-lg max-sm:text-sm opacity-70 font-semibold text-gray-800">{instructorName}</h2>
                </div>

                {/* Course Name */}
                <Link to={`/newest/${_id}`}>
                    <h3 className="text-lg max-sm:text-sm font-bold text-gray-900 mb-2">{courseName}</h3>
                </Link>
                ...
                <div className='flex justify-between  mb-4'>
                    <div className='flex items-center text-gray-600 text-sm'>
                        <CiTimer className="mr-2 text-xl" />
                        <h1 className='text-xl max-sm:text-sm'>{time}</h1>
                    </div>
                    <div className='flex items-center text-gray-600 text-sm'>
                        <CiCalendar className="mr-2 text-xl" />
                        <h1 className='text-xl max-sm:text-sm'>{date}</h1>
                    </div>
                </div>
